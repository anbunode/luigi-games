...
type DraftOrderCreateFormProps = {
  onCancel?: () => void
}

const DraftOrderCreateForm = ({ onCancel }: DraftOrderCreateFormProps) => {
  const navigate = useNavigate()
  const [email, setEmail] = useState("")
  const [regionId, setRegionId] = useState(NO_REGION)
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      navigate(`/borradores/${data.draft_order.id}`)
    },
    onError: (error) => {
      toast.error(
...
export default DraftOrderCreateForm
