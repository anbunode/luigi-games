...
const DETAIL_FIELDS =
  "id,display_id,status,email,customer_id,created_at,updated_at,currency_code,total,subtotal,tax_total,shipping_total,discount_total,metadata,summary,+region.id,+region.name,+region.currency_code,+sales_channel.id,+sales_channel.name,+customer.id,+customer.email,+customer.first_name,+customer.last_name,*items,+items.detail,+items.thumbnail,+items.variant_id"
...
export function formatDraftItemUnitPrice(
  item: DraftOrderItem,
  currency?: string
): string {
  const unitPrice = item.unit_price ?? item.detail?.unit_price
