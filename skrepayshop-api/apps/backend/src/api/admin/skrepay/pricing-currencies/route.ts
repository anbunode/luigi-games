 ...
 * Moneda base + monedas de regiones (sin duplicar). Solo para precios de productos.
 */
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!storeId) {
    res.json({ supported_currencies: [], store_id: null, region_currency_codes: [] })
    return
  }
  ...
  res.json({
    ...
    store_id: storeId,
    region_currency_codes: regionCurrencyCodes,
  })
}
